...
def inject_custom_css() -> None:
    st.markdown(
        ...
        .score-low { background: linear-gradient(90deg, #16A34A, #22C55E); }
        .score-medium { background: linear-gradient(90deg, #F59E0B, #F97316); }
        .score-high { background: linear-gradient(90deg, #DC2626, #EF4444); }
...
def clamp(value: float, low: float = 0.0, high: float = 100.0) -> float:
    """Keep a numeric score between low and high."""
    try:
        value = float(value)
...
def negative_sentiment_risk(value: float) -> float:
    """Convert sentiment from -1..1 into a 0..100 risk score, only penalising negative sentiment."""
    try:
        value = float(value)
    except (TypeError, ValueError):
        return 0.0
    return clamp(abs(min(value, 0.0)) * 100)


def account_risk_family_scores(row: pd.Series) -> dict[str, dict[str, object]]:
    """
    Explainability layer: calculates intuitive 0-100 risk scores by signal family.
    These scores are not the ML prediction itself; they are a transparent business reading
    of which risk families are contributing most strongly to the account situation.
    """
    usage_score = np.mean([
        100 - clamp(row["tool_activity_score"]),
        100 - clamp(row["feature_adoption_score"]),
        clamp(abs(min(float(row["usage_change_vs_prev_quarter"]), 0)) * 2.5),
        clamp(float(row["days_since_last_login"]) / 45 * 100),
        clamp((12 - float(row["login_count_30d"])) / 12 * 100),
    ])

    renewal_intent_risk = {"positive": 0, "neutral": 55, "negative": 100}.get(str(row["renewal_intent"]).lower(), 35)
    renewal_score = np.mean([
        clamp((60 - float(row["renewal_due_days"])) / 60 * 100),
        renewal_intent_risk,
        100 if int(row["in_grace_period"]) == 1 else 0,
        40 if int(row["renewed_last_quarter"]) == 0 else 0,
    ])

    support_score = np.mean([
        clamp(float(row["tickets_last_quarter"]) / 12 * 100),
        clamp(float(row["reopened_tickets"]) / 4 * 100),
        clamp(float(row["avg_resolution_days"]) / 12 * 100),
        clamp((5 - float(row["csat_support"])) / 5 * 100),
    ])

    sentiment_score = np.mean([
        negative_sentiment_risk(row["sentiment_csm"]),
        negative_sentiment_risk(row["sentiment_support"]),
    ])

    feedback_score = np.mean([
        clamp(float(row["critical_feedback_count_90d"]) / 2 * 100),
        clamp(float(row["unresolved_feedback_count"]) / 6 * 100),
        clamp(float(row["avg_unresolved_feedback_age_days"]) / 90 * 100),
        100 - clamp(row["roadmap_alignment_score"]),
        negative_sentiment_risk(row["feedback_sentiment_score"]),
        100 if int(row["competitor_mentioned_in_feedback"]) == 1 else 0,
        100 if int(row["feedback_linked_to_renewal"]) == 1 else 0,
    ])

    bug_score = np.mean([
        clamp(float(row["critical_bugs_reported_90d"]) / 2 * 100),
        clamp(float(row["open_bugs_count"]) / 5 * 100),
        clamp(float(row["stuck_bugs_count"]) / 2 * 100),
        clamp(float(row["avg_bug_resolution_days"]) / 21 * 100),
        clamp(float(row["oldest_open_bug_days"]) / 90 * 100),
        clamp(float(row["bug_reopen_count"]) / 3 * 100),
        100 if int(row["data_accuracy_bug_flag"]) == 1 else 0,
        80 if int(row["workaround_available_flag"]) == 0 else 0,
        negative_sentiment_risk(row["bug_sentiment_score"]),
        100 if int(row["bug_linked_to_renewal_flag"]) == 1 else 0,
    ])

    scores = {
        "Usage & adoption": {
            "score": round(float(usage_score), 1),
            "note": "Low activity, weak adoption, declining usage, or long inactivity.",
        },
        "Renewal context": {
            "score": round(float(renewal_score), 1),
            "note": "Renewal timing, intent, grace period, and recent renewal status.",
        },
        "Support experience": {
            "score": round(float(support_score), 1),
            "note": "Ticket volume, reopened cases, resolution time, and support CSAT.",
        },
        "Sentiment": {
            "score": round(float(sentiment_score), 1),
            "note": "Negative tone in CSM or support interactions.",
        },
        "Feedback & roadmap": {
            "score": round(float(feedback_score), 1),
            "note": "Critical or unresolved feedback, roadmap mismatch, and renewal-linked requests.",
        },
        "Bugs & reliability": {
            "score": round(float(bug_score), 1),
            "note": "Open, stuck, reopened, data accuracy, or no-workaround bug friction.",
        },
    }
    return scores


def family_score_level(score: float) -> str:
    if score >= 65:
        return "high"
    if score >= 35:
        return "medium"
...
def family_score_html(name: str, score: float, note: str) -> str:
    level = family_score_level(score)
    width = clamp(score)
    return f"""
    <div class="family-score-card">
        <div class="family-score-header">
            <div class="family-score-name">{html.escape(name)}</div>
            <div class="family-score-value">{score:.1f}</div>
        </div>
        <div class="score-bar-track">
            <div class="score-bar-fill score-{level}" style="width: {width:.1f}%"></div>
        </div>
        <div class="family-score-note">{html.escape(note)}</div>
    </div>
    """


def portfolio_family_risk_scores(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for _, account in df.iterrows():
        scores = account_risk_family_scores(account)
        rows.append({family: info["score"] for family, info in scores.items()})
    if not rows:
        return pd.DataFrame(columns=["Risk family", "Average family risk score"])
    score_df = pd.DataFrame(rows)
    out = (
        score_df.mean()
        .sort_values(ascending=False)
        .reset_index()
        .rename(columns={"index": "Risk family", 0: "Average family risk score"})
    )
    out["Average family risk score"] = out["Average family risk score"].round(1)
    return out


def build_structured_action_plan(row: pd.Series) -> dict[str, str]:
    scores = account_risk_family_scores(row)
    top_family = max(scores.items(), key=lambda item: item[1]["score"])[0]
    second_family = sorted(scores.items(), key=lambda item: item[1]["score"], reverse=True)[1][0]

    urgency = "Low"
    if row["risk_level"] == "High" or row["feedback_linked_to_renewal"] == 1 or row["bug_linked_to_renewal_flag"] == 1:
        urgency = "High"
    elif row["risk_level"] == "Medium" or row["renewal_due_days"] < 45:
        urgency = "Medium"

    owners = ["CSM"]
    if top_family in ["Bugs & reliability", "Feedback & roadmap"] or row["data_accuracy_bug_flag"] == 1:
        owners.extend(["Support Lead", "Product Owner"])
    if row["renewal_due_days"] < 45 or row["renewal_intent"] in ["neutral", "negative"]:
        owners.append("Account Owner")
    owners = list(dict.fromkeys(owners))

    business_impact = "The account currently shows manageable risk and should remain under regular monitoring."
    if row["mrr"] >= 5000 and row["risk_level"] in ["High", "Medium"]:
        business_impact = "High commercial exposure: the account has significant MRR and should receive proactive ownership."
    elif row["renewal_due_days"] < 30:
        business_impact = "Time-sensitive renewal exposure: the account is approaching renewal and needs a clear risk response."
    elif row["data_accuracy_bug_flag"] == 1:
        business_impact = "Trust exposure: a data accuracy issue may weaken confidence in the product and its outputs."

    action = "Maintain regular engagement and reinforce value realization."
    if top_family == "Bugs & reliability":
        action = "Escalate the bug context with Product/Engineering, confirm workaround status, agree customer update cadence, and request an ETA for resolution."
    elif top_family == "Feedback & roadmap":
        action = "Review critical feedback with Product, confirm roadmap alignment, prepare expectation management, and identify possible alternatives or workarounds."
    elif top_family == "Usage & adoption":
        action = "Launch an adoption recovery plan focused on the workflows and features most likely to restore product value."
    elif top_family == "Renewal context":
        action = "Run a renewal-risk check-in, clarify blockers, and align CSM/Account Owner on a retention plan before the renewal window closes."
    elif top_family == "Support experience":
        action = "Audit recent support friction, identify repeated cases, and assign a Support Lead to close the loop with the customer."
    elif top_family == "Sentiment":
        action = "Schedule a relationship recovery conversation to understand concerns and rebuild confidence with concrete follow-up actions."

    reason = f"The strongest visible risk family is {top_family}, followed by {second_family}."

    return {
        "Risk reason": reason,
        "Business impact": business_impact,
        "Recommended action": action,
        "Owner to involve": ", ".join(owners),
        "Urgency": urgency,
    }


def structured_action_plan_html(row: pd.Series) -> str:
    plan = build_structured_action_plan(row)
    cards = []

    for label, body in plan.items():
        body = str(body)
        if ":" in body and label != "Owner to involve":
            title, description = body.split(":", 1)
            title = title.strip()
        ...
        cards.append(
            f'<div class="playbook-card">'
            f'<div class="playbook-label">{html.escape(label)}</div>'
            f'<div class="playbook-title">{html.escape(title)}</div>'
            f'<div class="playbook-body">{html.escape(description)}</div>'
            f'</div>'
        )

    return f'<div class="playbook-grid">{"".join(cards)}</div>'


# ==========================================================
# Scenario simulator
# ==========================================================
def get_value(row: pd.Series, key: str, default=0):
...
def score_single_account(row: pd.Series, model, model_columns) -> float:
    account_df = pd.DataFrame([row.copy()])
    X_account = prepare_features(account_df, model_columns)
    return float(safe_predict_proba(model, X_account)[0])


def apply_scenario(row: pd.Series, scenario_key: str) -> pd.Series:
    simulated = row.copy()

    if scenario_key == "usage_adoption":
        simulated["tool_activity_score"] = max(float(get_value(simulated, "tool_activity_score", 0)), 75.0)
        simulated["feature_adoption_score"] = max(float(get_value(simulated, "feature_adoption_score", 0)), 72.0)
        simulated["login_count_30d"] = max(float(get_value(simulated, "login_count_30d", 0)), 20.0)
        simulated["key_reports_used_30d"] = max(float(get_value(simulated, "key_reports_used_30d", 0)), 6.0)
        simulated["days_since_last_login"] = min(float(get_value(simulated, "days_since_last_login", 999)), 3.0)
        simulated["usage_change_vs_prev_quarter"] = max(float(get_value(simulated, "usage_change_vs_prev_quarter", -100)), 10.0)

    elif scenario_key == "support_recovery":
        simulated["tickets_last_quarter"] = min(float(get_value(simulated, "tickets_last_quarter", 0)), 3.0)
        simulated["reopened_tickets"] = 0
        simulated["avg_resolution_days"] = min(float(get_value(simulated, "avg_resolution_days", 999)), 2.0)
        simulated["csat_support"] = max(float(get_value(simulated, "csat_support", 0)), 4.3)
        simulated["sentiment_support"] = max(float(get_value(simulated, "sentiment_support", -1)), 0.25)

    elif scenario_key == "renewal_confidence":
        simulated["renewal_intent"] = "positive"
        simulated["in_grace_period"] = 0
        simulated["renewed_last_quarter"] = 1

    elif scenario_key == "feedback_roadmap":
        simulated["critical_feedback_count_90d"] = 0
        simulated["unresolved_feedback_count"] = 0
        simulated["avg_unresolved_feedback_age_days"] = 0
        simulated["roadmap_alignment_score"] = max(float(get_value(simulated, "roadmap_alignment_score", 0)), 78.0)
        simulated["feedback_sentiment_score"] = max(float(get_value(simulated, "feedback_sentiment_score", -1)), 0.25)
        simulated["competitor_mentioned_in_feedback"] = 0
        simulated["feedback_linked_to_renewal"] = 0

    elif scenario_key == "bug_reliability":
        simulated["critical_bugs_reported_90d"] = 0
        simulated["open_bugs_count"] = 0
        simulated["stuck_bugs_count"] = 0
        simulated["avg_bug_resolution_days"] = min(float(get_value(simulated, "avg_bug_resolution_days", 999)), 2.0)
        simulated["oldest_open_bug_days"] = 0
        simulated["bug_reopen_count"] = 0
        simulated["data_accuracy_bug_flag"] = 0
        simulated["workaround_available_flag"] = 1
        simulated["bug_sentiment_score"] = max(float(get_value(simulated, "bug_sentiment_score", -1)), 0.25)
        simulated["bug_linked_to_renewal_flag"] = 0

    elif scenario_key == "best_case_recovery":
        for key in [
            "usage_adoption",
            "support_recovery",
            "renewal_confidence",
            ...
            simulated = apply_scenario(simulated, key)
        simulated["sentiment_csm"] = max(float(get_value(simulated, "sentiment_csm", -1)), 0.35)
        simulated["success_plan_active"] = 1
        simulated["strategic_review_done"] = 1
        simulated["csm_meetings_last_quarter"] = max(float(get_value(simulated, "csm_meetings_last_quarter", 0)), 2.0)

    return simulated


def scenario_definitions() -> list[dict[str, str]]:
    return [
        {
            "key": "usage_adoption",
            "title": "Improve usage & adoption",
            "description": "Raise activity, feature adoption, logins and recent report usage; reduce inactivity.",
        },
        {
            "key": "support_recovery",
            "title": "Resolve support friction",
            "description": "Reduce reopened tickets and resolution time while improving support CSAT and sentiment.",
        },
        {
            "key": "renewal_confidence",
            "title": "Strengthen renewal signal",
            "description": "Move renewal intent to positive and remove grace-period pressure.",
        },
        {
            "key": "feedback_roadmap",
            "title": "Resolve feedback friction",
            "description": "Close unresolved/critical feedback, improve roadmap fit and remove renewal-linked feedback risk.",
        },
        {
            "key": "bug_reliability",
            "title": "Resolve bug reliability issues",
            "description": "Clear stuck/open bugs, remove data accuracy issues and confirm workaround availability.",
        },
        {
            "key": "best_case_recovery",
            "title": "Best-case recovery package",
            "description": "Combine adoption recovery, support recovery, product feedback alignment and bug resolution.",
        },
    ]


def build_scenario_results(row: pd.Series, model, model_columns) -> pd.DataFrame:
    current_probability = float(row["churn_probability"])
    rows = []

    for scenario in scenario_definitions():
        simulated_row = apply_scenario(row, scenario["key"])
        simulated_probability = score_single_account(simulated_row, model, model_columns)
        delta = simulated_probability - current_probability
        rows.append(
            {
                "Scenario": scenario["title"],
                "What changes": scenario["description"],
                "Current probability": current_probability,
                "Simulated probability": simulated_probability,
                "Change pp": delta * 100,
                "Simulated risk level": risk_level(simulated_probability),
            }
        )

    return pd.DataFrame(rows).sort_values("Simulated probability", ascending=True).reset_index(drop=True)


def scenario_card_html(row: pd.Series) -> str:
    delta_pp = float(row["Change pp"])
    if delta_pp < -0.1:
        change_class = "scenario-improvement"
        change_text = f"{delta_pp:.1f} pp"
    elif delta_pp > 0.1:
        change_class = "scenario-worse"
        change_text = f"+{delta_pp:.1f} pp"
    else:
        change_class = "scenario-neutral"
        change_text = "No material change"

    return (
        f'<div class="scenario-card">'
        f'<div class="scenario-label">Simulated action</div>'
        f'<div class="scenario-title">{html.escape(str(row["Scenario"]))}</div>'
        f'<div class="scenario-body">{html.escape(str(row["What changes"]))}</div>'
        f'<div class="scenario-result-row">'
        f'<div><div class="scenario-prob">{fmt_pct(float(row["Simulated probability"]))}</div>'
        f'<div class="small-muted">New risk: {html.escape(str(row["Simulated risk level"]))}</div></div>'
        f'<span class="scenario-change {change_class}">{html.escape(change_text)}</span>'
        f'</div>'
        f'</div>'
    )


def scenario_simulator_html(scenario_results: pd.DataFrame) -> str:
    cards = [scenario_card_html(result_row) for _, result_row in scenario_results.iterrows()]
...
with tab1:
    ...
    section_header("Risk score by signal family", "Average 0-100 business risk score by family. This helps explain what is driving portfolio risk beyond the model probability.")
    family_scores_df = portfolio_family_risk_scores(filtered)
    fcols = st.columns(3)
    for idx, (_, score_row) in enumerate(family_scores_df.iterrows()):
        with fcols[idx % 3]:
            family_name = score_row["Risk family"]
            score_value = float(score_row["Average family risk score"])
            note = {
                "Usage & adoption": "Portfolio-level risk from usage intensity, adoption, trend, and inactivity.",
                "Renewal context": "Risk from renewal timing, renewal intent, grace period, and recent renewal status.",
                "Support experience": "Risk from ticket pressure, reopenings, resolution time, and support CSAT.",
                "Sentiment": "Risk from negative CSM or support sentiment.",
                "Feedback & roadmap": "Risk from unresolved feedback, critical asks, and roadmap mismatch.",
                "Bugs & reliability": "Risk from stuck bugs, data issues, no workaround, and bug resolution delays.",
            }.get(family_name, "Family-level risk score.")
            st.markdown(family_score_html(family_name, score_value, note), unsafe_allow_html=True)
...
with tab3:
    ...
    section_header("Risk driver score by family", "Transparent 0-100 business scores that explain which signal families are most concerning for this account.")
    account_scores = account_risk_family_scores(row)
    score_cols = st.columns(3)
    for idx, (family_name, info) in enumerate(account_scores.items()):
        with score_cols[idx % 3]:
            st.markdown(
                family_score_html(family_name, float(info["score"]), str(info["note"])),
                unsafe_allow_html=True,
            )

    section_header("Structured action plan", "A practical playbook generated from the strongest visible risk families.")
    st.markdown(structured_action_plan_html(row), unsafe_allow_html=True)

    section_header("Scenario Simulator", "Estimate how the predicted churn probability may change if specific risk drivers improve.")
    st.markdown(
        '<div class="scenario-note"><strong>How to read this:</strong> each scenario changes selected account signals and reruns the model for the same account. These are directional simulations, not guaranteed outcomes.</div>',
        unsafe_allow_html=True,
    )
    scenario_results = build_scenario_results(row, model, model_columns)
    st.markdown(scenario_simulator_html(scenario_results), unsafe_allow_html=True)

    scenario_table = scenario_results.copy()
    scenario_table["Current probability"] = scenario_table["Current probability"].map(fmt_pct)
    scenario_table["Simulated probability"] = scenario_table["Simulated probability"].map(fmt_pct)
    scenario_table["Change pp"] = scenario_table["Change pp"].map(lambda x: f"{x:+.1f} pp")
...
with tab4:
    ...
    section_header("Risk family explainability layer", "The account detail also includes 0-100 family scores. These are not the ML model itself; they are a transparent business layer that explains where the visible risk is concentrated.")
    e1, e2, e3 = st.columns(3)
    with e1:
        st.markdown("""
        <div class="method-card">
            <div class="method-label">Usage example</div>
            <div class="method-title">Usage & adoption risk score</div>
            <div class="method-body">Combines product activity, feature adoption, usage trend, days since last login, and login frequency into a readable 0-100 risk signal.</div>
        </div>
        """, unsafe_allow_html=True)
    with e2:
        st.markdown("""
        <div class="method-card">
            <div class="method-label">Product example</div>
            <div class="method-title">Feedback & roadmap risk score</div>
            <div class="method-body">Combines critical feedback, unresolved feedback, feedback age, roadmap alignment, feedback sentiment, competitor mentions, and renewal-linked feedback.</div>
        </div>
        """, unsafe_allow_html=True)
    with e3:
        st.markdown("""
        <div class="method-card">
            <div class="method-label">Reliability example</div>
            <div class="method-title">Bugs & reliability risk score</div>
            <div class="method-body">Combines open bugs, stuck bugs, critical bugs, bug resolution time, data accuracy issues, workaround availability, and renewal-linked bug friction.</div>
        </div>
        """, unsafe_allow_html=True)
