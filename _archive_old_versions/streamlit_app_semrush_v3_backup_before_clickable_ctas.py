...
# Guided tabs
mission_tab, queue_tab, insights_tab, briefing_tab, simulator_tab, playbook_tab, methodology_tab = st.tabs([
    "Mission Control",
    "Priority Queue",
    "Portfolio Insights",
    "Account Briefing",
...
with mission_tab:
    section_header("Mission Control", "Start here. The Copilot turns portfolio data into a clear CSM workflow.")
    st.markdown(mission_guide_html(), unsafe_allow_html=True)
    st.markdown(daily_brief_html(), unsafe_allow_html=True)
    st.markdown(mission_snapshot_grid_html(), unsafe_allow_html=True)
...
with queue_tab:
    section_header("CSM Priority Queue", "Start with P1 and P2. Tables are available only after the action-oriented cards.")
    q1, q2, q3, q4 = st.columns(4)
    with q1: metric_card("P1 · Act first", int((filtered["priority"] == "P1").sum()), "High risk and meaningful MRR")
    with q2: metric_card("P2 · Review this week", int((filtered["priority"] == "P2").sum()), "High risk or renewal-sensitive")
    with q3: metric_card("P3 · Monitor", int((filtered["priority"] == "P3").sum()), "Medium risk requiring prevention")
    with q4: metric_card("Monitor", int((filtered["priority"] == "Monitor").sum()), "Low risk / regular cadence")

    st.markdown('<div class="briefing-note"><strong>How to use this queue:</strong> do not start from the full table. Review P1 cards first, decide the next action, then use the Account Briefing for evidence and handoff details.</div>', unsafe_allow_html=True)
    ...
    for priority, title, expanded in [("P1", "P1 — Act first", True), ("P2", "P2 — Review this week", True), ("P3", "P3 — Monitor closely", False), ("Monitor", "Monitor — Regular cadence", False)]:
        with st.expander(title, expanded=expanded):
            render_priority_cards(filtered[filtered["priority"] == priority], max_cards=9)

    with st.expander("Full operational account table", expanded=False):
...
with briefing_tab:
    section_header("Account Briefing", "A call-ready briefing for one account: story, action, departments and evidence.")
    selected = st.selectbox("Select account", filtered["account_id"].tolist(), format_func=lambda x: f"{x} · {filtered.loc[filtered['account_id']==x, 'account_name'].iloc[0]}", key="briefing_account")
...
with simulator_tab:
    ...
    selected_sim = st.selectbox("Select account for simulation", filtered["account_id"].tolist(), format_func=lambda x: f"{x} · {filtered.loc[filtered['account_id']==x, 'account_name'].iloc[0]}", key="simulator_account")
    sim_row = filtered.loc[filtered["account_id"] == selected_sim].iloc[0]
    st.markdown(
        f'<div class="account-brief-hero"><div class="metric-label">Simulation target</div><div class="account-brief-title">{html.escape(str(sim_row["account_name"]))}</div><div class="account-risk-line"><span>{risk_badge(sim_row["risk_level"])}</span><span class="account-risk-value">{sim_row["churn_probability"]:.1%}</span></div><div class="account-story">{html.escape(account_story_summary(sim_row))}</div></div>',
        unsafe_allow_html=True,
    )
