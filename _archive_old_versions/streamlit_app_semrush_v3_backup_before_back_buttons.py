...
def render_clickable_mission_cards():
    cards = [
        ...
        },
        {
            ...
            "button": "Open product friction cases",
            "tone": "amber",
    ...
    for i, card in enumerate(cards):
        ...
        with cols[i]:
            st.markdown(
                f'<div class="mission-card" style="border-top:5px solid {color};">'
...
if active_view == "Mission Control":
    ...
    selected = st.selectbox("Select account", briefing_source["account_id"].tolist(), format_func=lambda x: f"{x} · {briefing_source.loc[briefing_source['account_id']==x, 'account_name'].iloc[0]}", key="briefing_account")
    ...
    selected_sim = st.selectbox("Select account for simulation", sim_source["account_id"].tolist(), format_func=lambda x: f"{x} · {sim_source.loc[sim_source['account_id']==x, 'account_name'].iloc[0]}", key="simulator_account")
