...
def render_ranked_story_bars(df, label_col, value_col, value_formatter, max_value=None, hot_threshold=None):
    ...
    if max_value is None:
        max_value = float(view[value_col].max()) if float(view[value_col].max()) > 0 else 1.0
    ...
    for _, r in view.iterrows():
        ...
        rows.append(
            f"""
            <div class="story-row">
                <div class="story-name">{html.escape(label)}</div>
                <div class="story-track"><div class="story-fill {css}" style="width:{width:.1f}%"></div></div>
                <div class="story-value">{html.escape(value_formatter(val))}</div>
            </div>
            """
        )
    st.markdown('<div class="story-bar-list">' + ''.join(rows) + '</div>', unsafe_allow_html=True)
